...
{   //创建一个上下方向分割的JSplitPane对象
	...
	String[]str1=new String[9];//声明字符串数组
	String sql;
	String sql2;
	String sql3;
	DataBase db;
	...
	private JButton[] jbArray={//设置JButton按钮的文本
	    ...
	    new JButton("查找全部学生信息")
	};
	Vector<String> head = new Vector<String>();
	{//创建标题
		head.add("学号");head.add("姓名");head.add("年龄");
		head.add("性别");head.add("班级");head.add("院系");
		head.add("电话号码");head.add("密码");head.add("借书权限");
	}
	...
	public void deleteStudent() throws SQLException{
		...
		try{	
		 ...
		 if((v.get(i)).equals("")){//若Record表中有，则输出提示对话框
		   ...
		   if(db.rs.next()){
			try{	
				  ...
				  while(db.rs.next()){
				       ...
				       sql2="update book,orderlist set ordered='否' where book.bookno=orderlist.bookno and orderlist.StuNo="+Integer.parseInt(stuno);
			           db=new DataBase();
			           db.updateDb(sql2); 
