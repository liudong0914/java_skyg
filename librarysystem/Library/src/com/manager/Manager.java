...
public class Manager extends JPanel implements ActionListener
{
	...
	public Manager(String mgNo)
	{
		...
		{
			jpt.add(jbArray[i]);
			jbArray[i].setBounds(140+140*i,70,100,25);
	...
	public void actionPerformed(ActionEvent e){
		...
		try{
			...
			if(string.equals("1")){   p++;
				...
				if(e.getSource()==jbArray[4]){//执行查询操作
			...
			if(p==0){ //说明该管理员是普通管理员，没有修改管理员信息的权限
				...
				if(jtxt.equals("")){//管理员名输入为空，提示
					...
					if(e.getSource()==jbArray[2]){//执行修改操作
						...
						if(row>=0){
							...
							sql="update manager set password='"
							     +str[4]+"' where mgNo="+Integer.parseInt(str[0].trim());
					...
					JOptionPane.showMessageDialog(this,	"你不能查看,修改别人的信息,请重新输入！",
				        "消息",JOptionPane.INFORMATION_MESSAGE);
				    return;					
				}
			}
	...
	public void insertManager(){
		...
		if(!str1[0].equals("")&&!str1[1].equals("")&&!str1[2].equals("")&&!str1[3].equals("")&&!str1[4].equals("")){
			if(!str1[0].matches("^\\d+$")){//判断管理员名的格式
				JOptionPane.showMessageDialog(this,	"管理员ID只能为数字！！！",
				        "消息",JOptionPane.INFORMATION_MESSAGE);
				return;							
			}
			int temp = Integer.parseInt(str1[0]);			
			sql="insert into manager(mgNo,mgName,mgTel,Permitted,password) values("
			          +temp+",'"+str1[1]+"','"+str1[2]+"','"+str1[3]+"','"+str1[4]+"')";//执行插入功能
			db=new DataBase();
			...
			JOptionPane.showMessageDialog(this,	"管理员信息不能为空！！！",
						        "消息",JOptionPane.INFORMATION_MESSAGE);
						        return;	
		}
	}
	public void deleteManager(){
		...
		if(mgNo.equals("")){//为空提示
			JOptionPane.showMessageDialog(this,	"管理员ID不能为空！！！",
						        "消息",JOptionPane.INFORMATION_MESSAGE);
			return;			
		}
	...
	public void updateManager(){
		...
		int row = jt.getSelectedRow();
		if(row>=0){
			for(int i=0;i<5;i++){//得到所选内容
				str[i]=jt.getValueAt(row,i).toString();
			}
			sql="update manager set mgNo='"+str[0]+"',mgName='"+str[1]
			   +"',mgTel='"+str[2]+"',Permitted='"+str[3]+"',password='"+str[4]+"' where mgNo="+Integer.parseInt(str[0].trim());
			db=new DataBase();
			db.updateDb(sql);
			JOptionPane.showMessageDialog(this,"修改成功！！",
			                      "消息!!",JOptionPane.INFORMATION_MESSAGE);
		...
		db.selectDb(sql);				   
	     try{//对结果集进行异常处
			 Vector<Vector> vtemp = new Vector<Vector>();
			  while(db.rs.next()){
		       	Vector<String> v = new Vector<String>();
				for(int i=1;i<=5;i++){//顺序达到所搜到的结果中的各项记录
    ...
    public void table(){
    	try{
			 ...
			 while(db.rs.next()){
			 	k++;
		       	Vector<String> v = new Vector<String>();
				for(int i=1;i<=5;i++){//顺序找到所搜到的结果中的各项记录
					String str=db.rs.getString(i).trim();
					str=new String(str.getBytes("gb2312"));
					v.add(str);	
				}
