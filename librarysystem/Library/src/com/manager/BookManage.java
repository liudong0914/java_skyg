...
public class BookManage extends JPanel implements ActionListener
{
	...
	public void deleteBook(){
		...
		sql="select * from book where BookNO="+Integer.parseInt(bookno);
		db=new DataBase();
		...
		try{	
		 ...
		 if((v.get(5)).equals("是")){//若borrowlist表中有该书号，则输出提示对话框
					JOptionPane.showMessageDialog(this,	"此书没有归还，不能删除此书",
								        "消息",JOptionPane.INFORMATION_MESSAGE);
					return;	
				}	
		if((v.get(6)).equals("是")){//若borrowlist表中有该书号，则输出提示对话框
				JOptionPane.showMessageDialog(this,	"此书已被预约，不能删除此书",
							        "消息",JOptionPane.INFORMATION_MESSAGE);
				return;	
			}
			else{
		sql="delete from book where BookNO="+Integer.parseInt(bookno);
		db.updateDb(sql);//若borrowlist表中无此书号则删除此书	
		JOptionPane.showMessageDialog(this,	"此书已成功删除！",
		        "消息",JOptionPane.INFORMATION_MESSAGE);
	}
