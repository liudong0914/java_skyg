...
public class Overdue extends JPanel implements ActionListener
{
	...
	private JLabel jl1=new JLabel("请输入已交欠款");
	private JTextField jtf1=new JTextField();
	//创建按钮
	private JButton jb=new JButton("交费");
	private JButton jb1=new JButton("欠费查询");
	public Overdue()
	{
   	...
   	public void actionPerformed(ActionEvent e)
	{
		...
		String sql="select OverTime from overdue where StuNO="
		...
		if(e.getSource()==jb1){//事件源为"查询欠款"按钮
			...
			if(k==JOptionPane.YES_OPTION){//选择对话框，当选择"是"时，将提示操作步骤
				float ii=Float.parseFloat(jtf1.getText().trim());		
				if(ii<(day*0.1)){
					sql="update overdue set overtime=overtime-"+ii/0.1+" where stuNO="+Integer.parseInt(sno);
					db=new DataBase();
					...
					if(i==1){//输出还应该缴费数的提示
			      	...
			      	JOptionPane.showMessageDialog(this,"你已成功交费"+day*0.1+"元",
										"消息",JOptionPane.INFORMATION_MESSAGE);
					jtf.setText("");
					sql="delete from overtime where StuNO="+Integer.parseInt(sno);
					db.updateDb(sql);//缴纳了所有欠款，更新overtime表信息
