...
public class BorrowBook extends JPanel implements ActionListener{
    ...
    public BorrowBook(String StuNo)
    {   this.StuNo=StuNo;
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==jb2){
			if(!(StuNo.equals(jtxt4.getText()))){//为输入为空的情况进行处理
			...
			if(jtxt3.getText().equals("")){//为输入为空的情况进行处理
				JOptionPane.showMessageDialog(this,"输入不能为空，请重新输入！！！",
				                      "信息",JOptionPane.INFORMATION_MESSAGE);
				return;
			}
			...
			try{
				if(!(db.rs.next())){//若学号错误，输出提示对话框
					...
					if(db.rs.getString(9).trim().equals("否")){//若无权限则输出提示对话框
						JOptionPane.showMessageDialog(this,"你没有借书权限！！！","消息",
					                              JOptionPane.INFORMATION_MESSAGE);
					}
						...
						if(jtxt3.getText().equals("")){//为输入为空的情况进行处理
							JOptionPane.showMessageDialog(this,"输入不能为空，请重新输入！！！",
							                      "信息",JOptionPane.INFORMATION_MESSAGE);
							return;}
						else{		sql="select * from Book where BookNO="
						...
						db.selectDb(sql);
							...
							for(int i=1;i<=7;i++){//顺序达到所搜到的结果中的各项记录
								if(i==5){//
									...
									str6=new String(str6.getBytes("gb2312"));
									v.add(str6);
								}
								if(i==6){//
									str7=db.rs.getString(i+1);
									str7=new String(str7.getBytes("gb2312"));
									v.add(str7);
								}
								if(i==2){//
								...
								if(i==3){//
									...
									author=new String(author.getBytes("gb2312"));
									v.add(author);	
								}	
								if(i==1){//
							...
							if(jrbArray[0].isSelected()){//选择了借阅图书
								if(str6.trim().equals("是")){//用户想借的书已经被借走，输出提示信息框
							    	...
							    	//检验借阅的用户和预约的用户是否为同一人
							    	sql="select * from Orderlist where BookNO="
										    +Integer.parseInt(jtxt3.getText().trim());
										db.selectDb(sql);
										db.rs.next();
										//int str8=db.rs.getInt(5);
						                String str8=db.rs.getString(5);
						                int s=Integer.parseInt(str8);
										//同一人时
										if(stuNo==s)
										{
											sql="DELETE FROM orderlist where BookNo="+Integer.parseInt(jtxt3.getText().trim());
											db.updateDb(sql);
											Date now =new Date();
											...
											sql="insert into borrowlist values("+Integer.parseInt(jtxt3.getText().trim())+","
							...
							if(jrbArray[1].isSelected()){//选择了预约图书
								if(str7.trim().equals("是")){	//该书已经被预约，输出提示信息框
									...
									sql="insert into orderlist values("+Integer.parseInt(jtxt3.getText().trim())
										+",'"+bookName+"',"+Integer.parseInt(jtxt4.getText().trim())+",'" +stuName+"')";
									db.updateDb(sql);
								}
							}
