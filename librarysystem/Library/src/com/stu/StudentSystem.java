...
public class StudentSystem extends JFrame implements ActionListener{
	DefaultMutableTreeNode[] dmtn={//创建节点数组
		...
		new DefaultMutableTreeNode(new NodeValue("个人信息查询")),
	    ...
	    new DefaultMutableTreeNode(new NodeValue("图书借阅")),
	    new DefaultMutableTreeNode(new NodeValue("图书挂失")),
	    new DefaultMutableTreeNode(new NodeValue("修改密码")),
	...
	public StudentSystem(String StuNo){
    	...
    	jt.addTreeSelectionListener(new TreeSelectionListener(){//用内部类显示树的各选择节点
			public void valueChanged(TreeSelectionEvent e){
				...
				else if(cnv.value.equals("图书借阅")){
					cl.show(jp,"BorrowBook");//显示"借阅预约图书"界面
				}
				else if(cnv.value.equals("图书挂失")){
					cl.show(jp,"GuaShi");//显示"归还挂失图书"界面
				}
				else if(cnv.value.equals("修改密码")){
					cl.show(jp,"cp");//显示"归还挂失图书"界面
				}
				else if(cnv.value.equals("个人信息查询")){
					cl.show(jp,"si");//显示"归还挂失图书"界面
				}
				else if(cnv.value.equals("借阅信息查询")){
					cl.show(jp,"bi");//显示"归还挂失图书"界面
				}
    	...
    	for(int i=1;i<4;i++){dmtn[0].add(dmtn[i]);}//给"图书馆信息管理系统"节点添加子节点
