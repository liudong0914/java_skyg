...
public class GuaShi extends JPanel implements ActionListener
{
	...
	{   this.StuNo=StuNo;
	 ...
	 jpt.add(jtxt);
   	...
   	{   
   		if(!(StuNo.equals(jtxt.getText()))){//为输入为空的情况进行处理
   	...
   	public void loselist(int row){
		...
		//找到最大的丢失记录号
		sql="select MAX(LbNO) from loselist";
		db.selectDb(sql);
		try{
			if(db.rs.next()){
		...
		sql="insert into loselist values("+lbno+","+sno+","+bno+",'"+bname+"')";//向丢书记录表中插入记录
		db.updateDb(sql);
		sql="select BookNo from orderlist where BookNO="+bno;//检查预约表中是否预约该书，若有，删除
		db.selectDb(sql);
		try{
			while(db.rs.next()){//删除记录
				sql="delect from orderlist where BookNO="+bno;
				db.updateDb(sql);
			}
		}
		catch(Exception e){e.printStackTrace();}
		sql="select BookNo from overdue where BookNO="+bno;//检查超期表中是否有该书，若有，删除
		db.selectDb(sql);
		try{
			while(db.rs.next()){//从超期表中删除记录
				sql="delect from overdue where BookNO="+bno;
				db.updateDb(sql);
			}
		}
		catch(Exception e){e.printStackTrace();}
		sql="delete from borrowlist where BookNO="+bno;//从借书表中将丢失图书的记录删除
		db.updateDb(sql);
